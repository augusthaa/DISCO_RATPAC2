...
VertexGen_Muon::VertexGen_Muon(const char *arg_dbname)
  ...
  _particle = "e-";
  _spectrum = "flat";
  _lspec = DB::Get()->GetLink("MUON_SPECTRUM", _spectrum); // default
...
///-------------------------------------------------------------------------
void VertexGen_Muon::GeneratePrimaryVertex(G4Event *event, G4ThreeVector &dx,
                                           G4double dt) {
  ...
  // cosmic muon direction

  // theta and phi
  G4double v = G4UniformRand();
  G4double w = G4UniformRand();

  G4double cosTheta = std::pow(v, 0.25);
...
void VertexGen_Muon::SetState(G4String newValues) {
  if (newValues.length() == 0) {
    ...
    info << "Current state of this VertexGen_Muon:" << newline << " \""
         << GetState() << "\"" << newline << newline;
    info << "Format of argument to VertexGen_Muon::SetState: " << newline
  ...
  if (is.fail() || pname.length() == 0) {
    Log::Die("VertexGen_Muon: Incorrect vertex setting " + newValues);
  }
    ...
    if (A > 0) {
      for (Z = 1; Z <= GLG4VertexGen_Gun::numberOfElements; Z++) {
        ...
        if (Z <= GLG4VertexGen_Gun::numberOfElements) {
          ...
          info << " Muon Vertex: Setting ion with A = " << A << " Z = " << Z
               << newline;
        }
      }
    }
    if (newTestGunG4Code == NULL) {
      warn << "Muon Vertex: Could not find particle type " << pname
           << " defaulting to electron " << newline;
      _particle = "e-";
      return;
    }
  } else {
    info << "Muon Vertex: Setting particle = " << pname << newline;
  }
  ...
  if (lspec) {
    _lspec = lspec;
    _spectrum = specname;
    info << "Muon Vertex: Setting spectrum " << specname << newline;
  } else {
    warn << "Could not find spectrum " << specname
         << " using default, flat spectrum " << newline;
  }
  ...
  if (is.fail()) {
    ...
    Elim_Ulo = Elo;
    Elim_Uhi = Ehi;
    info << "Limiting spectrum to range " << Elim_Ulo << " - " << Elim_Uhi
         << newline;
  }
...
///-------------------------------------------------------------------------
void VertexGen_Muon::InitialiseSpec() {
  ...
  if (_emax - _emin <= 0 || _emin < 0 || _emax < 0) {
    ...
    Log::Die("VertexGen_Muon: Nonsensical energy range for spectrum " +
             _spectrum);
  }
  ...
  for (unsigned int istep = 1; istep < spec_E.size(); ++istep) {
    ...
    // of spectrum
    magsum += (spec_mag[istep] + spec_mag[istep - 1]) / 2.0 *
              (spec_E[istep] - spec_E[istep - 1]);
    spec_cummag.push_back(magsum);
  }

  // check that this works with the universal energy limits
  if ((_emax < Elim_Ulo) || (_emin > Elim_Uhi)) {
    Log::Die("VertexGen_Muon: selected spectrum does not lie within chosen "
             "energy range: " +
             _spectrum);
  }
  ...
  Elim_Tlo = Elim_Ulo;
  Elim_Thi = Elim_Uhi;
  info << "Muon " << _spectrum << " initialised " << newline;
}

///-------------------------------------------------------------------------
double VertexGen_Muon::SampleEnergy() {
  ...
  if (Ehi < _emax) {
    ...
    while (spec_E[istep] > Ehi) {
      ...
      info << "* " << istep << " " << stop << " " << spec_E[istep] << " " << Ehi
           << newline;
      istep--;
    }
  ...
  double spec_slope = (spec_mag[istep + 1] - spec_mag[istep]) /
                      (spec_E[istep + 1] - spec_E[istep]);
...
///-------------------------------------------------------------------------
void VertexGen_Muon::LimitEnergies(double Elo, double Ehi) {
  // Set the limits for the generated energy range
  // first check this makes sense
  if ((Elo > _emax) || (Elo > Elim_Uhi) || (Ehi < _emin) || (Ehi < Elim_Ulo) ||
      ((Ehi - Elo) <= 0)) {
    info << "Muon Vertex: temporary energy limits " << Elo << " - " << Ehi
         << " don't make sense, not applied " << newline << "spectrum in range "
         << _emin << " - " << _emax << " MeV with universal limits " << Elim_Ulo
         << " - " << Elim_Uhi << newline;
    return;
  }
  Elim_Tlo = Elo;
...
///-------------------------------------------------------------------------
double VertexGen_Muon::EMaximum() {
...
///-------------------------------------------------------------------------
double VertexGen_Muon::EMinimum() {
