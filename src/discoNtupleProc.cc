...
discoNtupleProc::discoNtupleProc() : Processor("discontuple") {
  outputFile = nullptr;
  outputTree = nullptr;
  metaTree = nullptr;
  ...
  DB *db = DB::Get();
  DBLinkPtr table = db->GetLink("IO", "discoNtupleProc");
  try {
  ...
  try {
    ...
    options.mutrackerhits = table->GetZ("include_mutrackerhits");
    // options.mtTrkAllHits = table->GetZ("include_mtTrkAllHits");
  } catch (DBNotFoundError &e) {
    ...
    options.mutrackerhits = false;
    // options.mtTrkAllHits = false;
  }
  if (options.digitizerfits) {
    ...
    for (const std::string &fitter_name : waveform_fitters) {
      try {
        waveform_fitter_FOMs[fitter_name] =
            table->GetSArray("waveform_fitter_FOM_" + fitter_name);
      } catch (DBNotFoundError &e) {
  ...
  for (const std::string &full_name : event_fitters) {
    if (event_fitter_FOMs.find(full_name) != event_fitter_FOMs.end())
      continue;
    std::set<std::string> fom_set;
    // Load FOMs declared under the base fitter name (applies to all tagged
    // variants)
    try {
      for (const std::string &fom :
           table->GetSArray("event_fitter_FOM_" + full_name)) {
        fom_set.insert(fom);
      }
    } catch (DBNotFoundError &e) {
      std::string fitter_name =
          DS::FitResult::GetFitterNameFromFullName(full_name);
      try {
        for (const std::string &fom :
             table->GetSArray("event_fitter_FOM_" + fitter_name)) {
          fom_set.insert(fom);
        }
      } catch (DBNotFoundError &e) {
        info << "No FOMs found for fitter " << full_name << " or "
             << fitter_name << ". No FOM will be saved for this fitter."
             << newline;
      }
    }
    event_fitter_FOMs[full_name] =
        std::vector<std::string>(fom_set.begin(), fom_set.end());
  }
  event_classifiers = table->GetSArray("event_classifiers");
  for (const std::string &full_name : event_classifiers) {
    if (event_classifier_FOMs.find(full_name) != event_classifier_FOMs.end())
      continue;
    std::set<std::string> fom_set;
    // Load FOMs declared under the base classifier name (applies to all tagged
    // variants)
    try {
      for (const std::string &fom :
           table->GetSArray("event_classifier_FOM_" + full_name)) {
        fom_set.insert(fom);
      }
    } catch (DBNotFoundError &e) {
      std::string classifier_name =
          DS::Classifier::GetClassifierNameFromFullName(full_name);
      try {
        for (const std::string &fom :
             table->GetSArray("event_classifier_FOM_" + classifier_name)) {
          fom_set.insert(fom);
        }
      } catch (DBNotFoundError &e) {
        info << "No FOMs found for classifier " << full_name << " or "
             << classifier_name << ". No FOM will be saved for this classifier."
             << newline;
      }
    }
    event_classifier_FOMs[full_name] =
        std::vector<std::string>(fom_set.begin(), fom_set.end());
  }
}

void discoNtupleProc::BeginOfRun(DS::Run *run) {
  info << "Running OUTNtuple BeginOfRun" << newline;
  DBLinkPtr table = DB::Get()->GetLink("IO", "discoNtupleProc");
  if (options.transittime) {
    std::vector<std::string> ttime_cfg =
        table->GetSArray("transittime_calculator_config");
    Log::Assert(ttime_cfg.size() == 2,
                "transittime_calculator_config should have 2 entries: path "
                "calculator ID and velocity calculator ID");
    fTransitTimeCalculator = std::make_unique<RAT::TransitTimeCalculator>(
        ttime_cfg[0], ttime_cfg[1]);
    info << "Initialized TransitTimeCalculator with calculator ID "
         << ttime_cfg[0] << " and velocity calculator ID " << ttime_cfg[1]
         << newline;
  }
}

bool discoNtupleProc::OpenFile(std::string filename) {
  ...
  if (options.digitizerfits) {
    for (const std::string &fitter_name : waveform_fitters) {
      outputTree->Branch(TString("fit_pmtid_" + fitter_name),
                         &wfmFitPmtID[fitter_name]);
      outputTree->Branch(TString("fit_time_" + fitter_name),
                         &wfmFitTime[fitter_name]);
      outputTree->Branch(TString("fit_charge_" + fitter_name),
                         &wfmFitCharge[fitter_name]);
      for (const std::string &fom_name : waveform_fitter_FOMs[fitter_name]) {
        outputTree->Branch(TString("fit_FOM_" + fitter_name + "_" + fom_name),
                           &wfmFitFOM[fitter_name][fom_name]);
      }
    }
  }
  for (const std::string &fitter_full_name : event_fitters) {
    ...
    outputTree->Branch(TString("x_" + fitter_full_name),
                       &fitvalues["x_" + fitter_full_name]);
    outputTree->Branch(TString("y_" + fitter_full_name),
                       &fitvalues["y_" + fitter_full_name]);
    outputTree->Branch(TString("z_" + fitter_full_name),
                       &fitvalues["z_" + fitter_full_name]);
    outputTree->Branch(TString("u_" + fitter_full_name),
                       &fitvalues["u_" + fitter_full_name]);
    outputTree->Branch(TString("v_" + fitter_full_name),
                       &fitvalues["v_" + fitter_full_name]);
    outputTree->Branch(TString("w_" + fitter_full_name),
                       &fitvalues["w_" + fitter_full_name]);
    outputTree->Branch(TString("energy_" + fitter_full_name),
                       &fitvalues["energy_" + fitter_full_name]);
    outputTree->Branch(TString("time_" + fitter_full_name),
                       &fitvalues["time_" + fitter_full_name]);
    outputTree->Branch(TString("validposition_" + fitter_full_name),
                       &fitvalids["validposition_" + fitter_full_name]);
    outputTree->Branch(TString("validdirection_" + fitter_full_name),
                       &fitvalids["validdirection_" + fitter_full_name]);
    outputTree->Branch(TString("validenergy_" + fitter_full_name),
                       &fitvalids["validenergy_" + fitter_full_name]);
    outputTree->Branch(TString("validtime_" + fitter_full_name),
                       &fitvalids["validtime_" + fitter_full_name]);
    for (const std::string &fom_name : event_fitter_FOMs[fitter_full_name]) {
      outputTree->Branch(TString(fom_name + "_" + fitter_full_name),
                         &fiteventFOMs[fitter_full_name][fom_name]);
    }
  }
  for (const std::string &classifier_full_name : event_classifiers) {
    // classifier names are specified as either classifiername__tag or just
    // classifiername.
    for (const std::string &fom_name :
         event_classifier_FOMs[classifier_full_name]) {
  ...
  if (options.mutrackerhits) {
    /*
    if (options.mtTrkAllHits){
      outputTree->Branch("mtHitDetID", &mtHitDetID);
      outputTree->Branch("mtHitEdep", &mtHitEdep);
      outputTree->Branch("mtHitTime", &mtHitTime);
      outputTree->Branch("mtHitX", &mtHitX);
      outputTree->Branch("mtHitY", &mtHitY);
      outputTree->Branch("mtHitZ", &mtHitZ);
    }
    */

    outputTree->Branch("mtPanelDetID", &mtPanelDetID);
    outputTree->Branch("mtPanelEdep", &mtPanelEdep);
    outputTree->Branch("mtPanelTime", &mtPanelTime);
    outputTree->Branch("mtPanelX", &mtPanelX);
    outputTree->Branch("mtPanelY", &mtPanelY);
    outputTree->Branch("mtPanelZ", &mtPanelZ);

    outputTree->Branch("mt_mcpdg", &mt_mcpdg);
    outputTree->Branch("mt_mcu", &mt_mcu);
    outputTree->Branch("mt_mcv", &mt_mcv);
    outputTree->Branch("mt_mcw", &mt_mcw);
    outputTree->Branch("mt_mcke", &mt_mcke);
  }
...
Processor::Result discoNtupleProc::DSEvent(DS::Root *ds) {
  if (!this->outputFile) {
  ...
  */
  ...
  if (options.mutrackerhits) {
    ...
    DS::MC *mcForTracker = ds->GetMC();
    if (mcForTracker->GetMCParticleCount() > 0) {
      DS::MCParticle *primary = mcForTracker->GetMCParticle(0);
      mt_mcpdg = primary->GetPDGCode();
      mt_mcke = primary->GetKE();
      TVector3 dir = primary->GetMomentum();
      if (dir.Mag() > 0) {
        mt_mcu = dir.X() / dir.Mag();
        mt_mcv = dir.Y() / dir.Mag();
        mt_mcw = dir.Z() / dir.Mag();
      }
    }

    // Raw per-step hits from the tracker SD
    const G4Event *g4ev = G4RunManager::GetRunManager()->GetCurrentEvent();
    G4HCofThisEvent *hce = g4ev ? g4ev->GetHCofThisEvent() : nullptr;

    static G4int hcID = -2; // -2 = not yet looked up, -1 = lookup failed
    if (hcID == -2) {
      hcID = G4SDManager::GetSDMpointer()->GetCollectionID("MuTrackerHits");
      if (hcID < 0) {
        warn << "discoNtupleProc: could not resolve MuTrackerHits collection ID"
             << newline;
      } else {
        info << "discoNtupleProc: MuTrackerHits collection ID = " << hcID
             << newline;
      }
    }
    ...
    if (hce && hcID >= 0) {
      auto *hitsCollection =
          static_cast<GeoMuTrackerSDHitsCollection *>(hce->GetHC(hcID));
      if (hitsCollection) {
        ...
        std::map<int, trackerHits> mtHits;

        for (size_t i = 0; i < hitsCollection->entries(); i++) {
          GeoMuTrackerSDHit *hit = (*hitsCollection)[i];
          int detID = hit->GetID();
          ...
          trackerHits &mtHitsPanel = mtHits[detID];
          mtHitsPanel.nhits += 1;
          mtHitsPanel.edep += edep;
          mtHitsPanel.avgTime += t;
          mtHitsPanel.avgX += x;
          mtHitsPanel.avgY += y;
          mtHitsPanel.avgZ += z;
          ...
          /*
          if (options.mtTrkAllHits) {
            mtHitDetID.push_back(hit->GetID());
            mtHitEdep.push_back(hit->GetEdep());
            mtHitTime.push_back(hit->GetTime());
            mtHitX.push_back(hit->GetPos().x());
            mtHitY.push_back(hit->GetPos().y());
            mtHitZ.push_back(hit->GetPos().z());
          }
          */
        }

        for (const auto &aPanel : mtHits) {
          const trackerHits &mtHitsPanel = aPanel.second;
          mtPanelDetID.push_back(aPanel.first);
          mtPanelEdep.push_back(mtHitsPanel.edep);
          mtPanelTime.push_back(mtHitsPanel.avgTime / mtHitsPanel.nhits);
          mtPanelX.push_back(mtHitsPanel.avgX / mtHitsPanel.nhits);
          mtPanelY.push_back(mtHitsPanel.avgY / mtHitsPanel.nhits);
          mtPanelZ.push_back(mtHitsPanel.avgZ / mtHitsPanel.nhits);
        }
      }
    }
  }
  ...
  if (options.transittime) {
    ...
    for (int id = 0; id < pmtinfo->GetPMTCount(); id++) {
      ...
      TransitTimeCalculator::Result ttime_result =
          fTransitTimeCalculator->Compute(event_vertex, pmt_pos);
  ...
  for (subev = 0; subev < ds->GetEVCount(); subev++) {
    ...
    timestamp = TTimeStamp_to_UnixTime(ev->GetUTC()) -
                TTimeStamp_to_UnixTime(runBranch->GetStartTime()) + triggerTime;
    ...
    for (DS::FitResult *fit : ev->GetFitResults()) {
      ...
      if (std::find(event_fitters.begin(), event_fitters.end(), full_name) ==
          event_fitters.end()) {
        info << "Fitter " << full_name
             << " not found in the list of requested fitters. Will not be "
                "written to the ntuple."
             << newline;
        continue;
      }
    ...
    for (DS::Classifier *clf : ev->GetClassifierResults()) {
      ...
      // Check the validity and write it out
      if (std::find(event_classifiers.begin(), event_classifiers.end(),
                    full_name) == event_classifiers.end()) {
        info << "Classifier " << full_name
             << " not found in the list of requested classifiers. Will not be "
                "written to the ntuple."
             << newline;
        continue;
      }
      for (const std::string &fom_name : event_classifier_FOMs[full_name]) {
        classifyeventFOMs[full_name][fom_name] =
            clf->GetClassificationResult(fom_name);
      }
    }
    ...
    if (ev->DigitizerExists()) {
      // Writing this information to meta, despite the fact that in principle
      // events could have varying values for these fields. Writing these values
      // to meta is much more convenient to use, so I'm going to assume that
      // this doesn't happen;)
    ...
    if (options.digitizerhits) {
      ...
      for (int pmtc : ev->GetAllDigitPMTIDs()) {
        ...
        digitVoltageOverThreshold.push_back(
            digitpmt->GetVoltageOverThreshold());
        ...
        if (options.digitizerfits) {
          ...
          for (std::string fitter_name : fitters) {
            DS::WaveformAnalysisResult *fit_result =
                digitpmt->GetOrCreateWaveformAnalysisResult(fitter_name);
            for (int hitidx = 0; hitidx < fit_result->getNPEs(); hitidx++) {
              ...
              wfmFitCharge[fitter_name].push_back(
                  fit_result->getCharge(hitidx));
              for (const std::string &fom_name :
                   waveform_fitter_FOMs[fitter_name]) {
                wfmFitFOM[fitter_name][fom_name].push_back(
                    fit_result->getFOM(fom_name, hitidx));
              }
            }
          }
        }
    ...
    if (options.digitizerwaveforms) {
      ...
      double readout_window_max =
          digitizer.GetNSamples() * digitizer.GetTimeStepNS();
      for (auto const &pair : digitizer.GetAllWaveforms()) {
        ...
        if (mc->GetMCPMTCount() == 0) {
          ...
          if (it == mcpmtid.end())
            ...
            double time_offset =
                channel_status->GetCableOffsetByPMTID(waveform_pmtid);
            DS::MCPMT *mcpmt = mc->GetMCPMT(it - mcpmtid.begin());
            for (int ipe = 0; ipe < mcpmt->GetMCPhotonCount(); ipe++) {
              DS::MCPhoton *mcph = mcpmt->GetMCPhoton(ipe);
              Double_t time = mcph->GetFrontEndTime() -
                              ev->GetCalibratedTriggerTime() + time_offset;
  ...
  // if( errorcode < 0 )
  //{
  //  Log::Die(std::string("discoNtupleProc: Error fill ttree, check disk
  //  space"));
  //}
  return Processor::OK;
}

void discoNtupleProc::EndOfRun(DS::Run *run) {
  if (outputFile) {
    outputFile->cd();
...
void discoNtupleProc::SetS(std::string param, std::string value) {
  if (param == "file") {
    this->defaultFilename = value;
  }
}

void discoNtupleProc::SetI(std::string param, int value) {
  ...
  if (param == "include_mutrackerhits") {
    options.mutrackerhits = value ? true : false;
  }
}
